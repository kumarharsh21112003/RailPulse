...
export default function MapView({ journey, className }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Zoom in to current train location by default
      if (followTrainMode) {
        map.flyTo({ center, zoom: 11, pitch: 40, duration: 1500 });
      }
    });

    // Disable follow mode when user interacts manually
    map.on('dragstart', (e) => {
      if (e.originalEvent) setFollowTrainMode(false);
    });
    map.on('zoomstart', (e) => {
      if (e.originalEvent) setFollowTrainMode(false);
    });
  ...
  useEffect(() => {
    ...
    if (map.getSource('route')) {
      ...
      map.addLayer({
        ...
        paint: { 'line-color': '#0284c7', 'line-width': 16, 'line-opacity': 0.3, 'line-blur': 12 },
      });
